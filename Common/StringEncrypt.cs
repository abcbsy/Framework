...
namespace Common
{
    public class StringEncrypt
    {
        ...
        public static string EncryptWithMD5(string data, string input_charset)
        {
            MD5 md5 = new MD5CryptoServiceProvider();
            byte[] hashData = md5.ComputeHash(Encoding.GetEncoding(input_charset).GetBytes(data));
            StringBuilder sb = new StringBuilder(32);
            for (int i = 0; i < hashData.Length; i++)
            {
                sb.Append(hashData[i].ToString("x").PadLeft(2, '0'));
            }
            return sb.ToString();
        }
